...
export class ProductsViewComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        // Update the subscription inside ngOnInit:
        this.pageService.list$.pipe(
            filter((x): x is ProductsList => x !== null),
            takeUntil(this.destroy$)
        ).subscribe(
            ({page, limit, sort, filterValues}) => {
                // FIXED: Added null/undefined check for filterValues
                this.filtersCount = filterValues ? Object.keys(filterValues).length : 0;
