...
/**
 * This service serves as a mediator between the PageDomainComponent, ProductsViewComponent and WidgetFiltersComponent components.
 */
@Injectable()
export class PageMarketplaceService {
    // isLoading
    ...
    protected optionsDiff(curr: ListOptions): ListOptions {
        ...
        // Removed the 'const' inside the loop that was shadowing the outer 'result'
        (['page', 'limit', 'sort'] as ('page' | 'limit' | 'sort')[]).forEach(key => {
            if (key in curr && (this as any)[key] !== (curr as any)[key]) {
                (result as any)[key] = (curr as any)[key];
            }
        });
        ...
        if ('filterValues' in curr) {
            ...
            Object.keys(filterValues).forEach(key => {
                if (this.filterValues[key] !== filterValues[key]) {
                    if (!result.filterValues) result.filterValues = {};
